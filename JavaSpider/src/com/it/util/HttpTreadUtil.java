 ...
 * @author zhangj
 *
 * 2016年11月10日  下午7:15:02
 */
public class HttpTreadUtil {

    private static Log log = LogFactory.getLog(HttpTreadUtil.class);
    /**
    ...
    public static String[] threadGet(List<Map<String, Object>> reqList) {
        ...
        try {
            ...
            for (int i = 0; i < reqList.size(); i++) {
                ...
                getThreads[i] = new GetThread(httpclient, get, (String) req.get("encode") ,i + 1);
            }
            //执行线程
...
//                        System.out.println(id+"：：：执行结果：：："+result);
    ...
    static class GetThread extends Thread implements Callable<String> {
        ...
        public GetThread(CloseableHttpClient httpClient, HttpGet httpget, String encode,int id) {
            this.httpClient = httpClient;
            this.context = new BasicHttpContext();
            this.httpget = httpget;
            this.id = id;
            charset = encode == null ? "UTF-8" : encode;
        }
